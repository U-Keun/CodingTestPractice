...
int solution(vector<vector<int>> grid, vector<int> d, int k) {
    ...
    col = grid[0].size();
    int l = d.size();
    vector<vector<ll>> dp(row * col, vector<ll>(row * col, 0));
    REP(i, 0, row * col - 1) dp[i][i] = 1;
    
    REP(i, 1, l) {
        vector<vector<ll>> next_dp(row * col, vector<ll>(row * col, 0));
        REP(j, 0, row * col - 1) {
            ...
            REP(k, 0, 3) {
                ...
                if (isValidPos(r, c) && grid[r][c] == grid[curRow][curCol] + d[i - 1]) {
                    REP(m, 0, row * col - 1) {
                        next_dp[m][r * col + c] += dp[m][j];
                        next_dp[m][r * col + c] %= MOD;
                    }
                }
            }
        }
    ...
    vector<vector<ll>> result = matrix_power(dp, k);
    ll answer = 0;
    REP(i, 0, row * col - 1) {
        REP(j, 0, row * col - 1) {
            answer += result[i][j];
