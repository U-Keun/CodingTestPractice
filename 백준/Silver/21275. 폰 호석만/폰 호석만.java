...
public class Main {
    public static void main(String[] args) throws IOException {
        ...
        Long[] listA = new Long[36 - minSysA + 1];
        Long[] listB = new Long[36 - minSysB + 1];
        Long calc;
        char c;
        int length;
        ...
        for (int i = minSysB; i < 37; i++) {
            ...
            for (int j = 0; j < length; j++) {
                ...
                long degree = Math.round(Math.pow(i, length - 1 - j));
                if (c >= 'a') {
                    calc += (B.charAt(j) - 'a' + 10) * degree;
                } else {
                    calc += (B.charAt(j) - '0') * degree;
                }
            }
            listB[i - minSysB] = calc;
        }
