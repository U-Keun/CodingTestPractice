...
int main() {
    ...
    frequency.resize(NUM_LIMIT * 2 + 1, 0);
    ...
    for (const auto &q : queries) {
        ...
        while (cur_right < r) add_frequency(NUM_LIMIT + arr[++cur_right]);
        while (cur_right > r) remove_frequency(NUM_LIMIT + arr[cur_right--]);
        while (cur_left < l) remove_frequency(NUM_LIMIT + arr[cur_left++]);
        while (cur_left > l) add_frequency(NUM_LIMIT + arr[--cur_left]);
